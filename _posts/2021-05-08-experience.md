...
过去，你记笔记的工具是什么？是传统的纸笔？还是电脑、手机上的Word？又或是轻量级的TXT,Markdown编辑器？
...
什么是页面？页面可以是一篇笔记，也可以是一个“文件夹”。目光转向我来左侧边栏中，看到`我的页面`四个字了吗？下面呈现出的就是当前工作空间的所有父页面。单击父页面左侧的`>`，页面会展开，显示出该页面下的子页面。传统的笔记软件创建出的页面都是平级的，而我来不同。独特的页面逻辑使得它更加易用。
...
一个「块」可以是一段文字，一张图片，也可以是一张表格，一段视频，甚至可以是一个页面。

双向链接

理解了工作空间，页面和块，你应该对wolai的操作逻辑有所了解，接下来我要介绍的是wolai独特的功能——双向链接

看一眼你的收藏夹：是不是有很多的教程已经吃灰多年？不反复阅读/观看，根本学不到知识。笔记也是如此，只有反复使用，才能发挥出它的价值。通过双向链接，wolai让信息自由地“生长”，而不是在文件夹里长眠。

超链接

我们可以通过超链接，与互联网进行交互：

<img src="https://img-blog.csdnimg.cn/20210424165757699.gif" alt="在这里插入图片描述" style="zoom:33%;" />



块引用和块嵌入

点击块前面的`::` 图标，在块菜单中选择“复制块引用链接”，在二级菜单中可以选择：

行内块引用——在一段文字中间引用其他块，行内块引用不可编辑，点击可以跳转到原块所在位置，某些块悬浮可以看到具体内容（比如代码、图片、表格等）。

![在这里插入图片描述](https://img-blog.csdnimg.cn/20210506221439648.gif#pic_center)

嵌入块引用——您可以使用一个独立的引用块来引用其他块，嵌入块引用可以直接编辑，但无法直接通过点击跳转到原块所在位置（但可以在块菜单中选择进行跳转），大部分嵌入块可以直接看到所有内容，包括子元素。

![在这里插入图片描述](https://img-blog.csdnimg.cn/20210506221513556.gif#pic_center)

选择后，引用链接将会复制到剪贴板。在需要插入引用的地方，粘贴即可完成块引用，灰常的方便。如果你打开了右侧边栏，则可以在两个页面之间通过拖拽创建块引用。具体操作请查看：[块引用与块嵌入 (wolai.com)](https://www.wolai.com/wolai/i1eTuzCbCDV4ymqaDPRN5w)

块引用可以视作超链接，而块嵌入则可以视作分身，当分身或本体任意一方发生改变时，所有的分身会跟着变。一处更新，处处更新，灰常的方便。

块引用和嵌入消灭了信息库中的冗余信息，让笔记之间相互联系，而页面关系图更能让人直观地通览笔记。多种视图、布局与筛选，展现出一张张知识网，方便复习。

<img src="https://img-blog.csdnimg.cn/20210424165828200.gif" alt="在这里插入图片描述" style="zoom:33%;" />

还有3D视图：

<img src="https://img-blog.csdnimg.cn/20210424165835344.gif" alt="在这里插入图片描述" style="zoom:33%;" />

无论哪一种方式，都能看到页面之间的联系，被你重复利用的笔记才能体现价值。

方便的操作方式

支持Markdown语法

我来是支持Markdown语法的，这一点让本来用typora记笔记的我立刻转到了我来。
...
<img src="https://img-blog.csdnimg.cn/2021042416591440.gif" alt="在这里插入图片描述" style="zoom:33%;" />



在我来中创建一个块，你可以点击左侧的加号选择块的类型，但这并不是最快捷的方式。

直接在新建的块中输入斜杠`/`即可，注意，不用切换到英文输入法，中文的顿号效果也是一样的。
...
在左侧边栏拖动页面即可对页面的位置进行调整，在页面中，拖动`::`即可对块进行调整

<img src="https://img-blog.csdnimg.cn/20210424170142531.gif" alt="在这里插入图片描述" style="zoom:33%;" />

模板&导入导出

在新页面中，我来会给出三个选项：回车开始编辑，从模板中心创建或是导入:

<img src="https://img-blog.csdnimg.cn/20210424170159305.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L2xid25ibmJuYm4=,size_16,color_FFFFFF,t_70" alt="在这里插入图片描述" style="zoom:33%;" />

模板中心有丰富的模板供你使用，几乎涵盖了所有类型的模板：

<img src="https://img-blog.csdnimg.cn/20210424170228566.gif" alt="在这里插入图片描述" style="zoom:33%;" />

导入也值得一提：

若要导入，也可以在右上的页面设置中选择导入

<img src="https://img-blog.csdnimg.cn/20210424170245934.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L2xid25ibmJuYm4=,size_16,color_FFFFFF,t_70" alt="在这里插入图片描述" style="zoom: 67%;" />

支持Markdown，TXT，OPML，HTML,Word以及印象笔记。涵盖了常用的文件格式，非常方便。

<img src="https://img-blog.csdnimg.cn/20210424170259145.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L2xid25ibmJuYm4=,size_16,color_FFFFFF,t_70" style="zoom: 50%;" >
...
说到对用户隐私的保护，大家总是不太放心。为了用户的隐私和数据安全，我来的开发人员竭尽所能。

数据加密

数据采取落盘加密存储，使用HTTPS加密传输保障数据安全。

云端存储

分布式云存储，完善的数据备份机制，专业级容灾能力，数据绝不丢失。

隐私保护

严格的权限、操作审计机制，保护个人信息，承诺绝不滥用隐私。

同步速度

如果你用过OneNote、Notion等软件，你会发现它们的同步速度真的是慢——到——离——谱————，服务器在国外，在国内用起来真的很蛋疼。而我来就完全不一样。电脑端按下`Ctrl+S`保存之后，等待2-3秒。在手机端看一眼，已经同步完成了。我来在速度这方面没得说——天下武功，唯快不破。

『細節』

俗话说“细节决定成败”。可是在介绍一款工具的时候很少有具体介绍细节的，这样很容易让大家忽略了工具的一些亮点。在`我来`这款软件中，不但有高效的功能，而且在一些细节上同样进行了精雕细琢。

直达图片内的文字搜索

我来终于让我们不再怕重要的聊天截图放到笔记里不知所踪了，它搜索功能真的很强大：这张图片中有white这个单词，我们用CTRL+P调出搜索框，搜索white，立等可取，直接定位到图片所在的位置。

<img src="https://img-blog.csdnimg.cn/20210424170331533.gif" style="zoom:33%;" />
...
<img src="https://img-blog.csdnimg.cn/20210424170524726.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L2xid25ibmJuYm4=,size_16,color_FFFFFF,t_70" alt="在这里插入图片描述" style="zoom:33%;" />

智能的悬浮目录

<img src="https://img-blog.csdnimg.cn/20210424170532365.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L2xid25ibmJuYm4=,size_16,color_FFFFFF,t_70" alt="在这里插入图片描述" style="zoom:33%;" />

宽屏时显示完整悬浮目录，窄屏时自动切换为极简模式。这一点对窄屏用户真的很友好

代码连字显示

对程序员十分友好，（程序猿狂喜）我来特别选择了具有连字显示特性的等宽字体展示代码，提升代码可读性。

丰富的图标&题头图

我来拥有1700+Emoji 图标、1500+Font Awesome 图标和我来动态图标，从此告别图标焦虑，其中还包含了多种编程语言的Logo并且支持Pexels和Unsplash的图库查询。学习编程做笔记的时候加上对应语言的图标，这感觉***针~布~戳~***
...
我来是支持打开右侧边栏的，把某个页面放在右侧，可以直接拖动块，轻松进行引用和嵌入。

页面设置

单击右上方的`...`，进入页面设置，你会发现暗黑模式（Good Night 模式），辅助提示，自适应宽度等等贴心的小功能，够细。

<img src="https://img-blog.csdnimg.cn/20210424171245704.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L2xid25ibmJuYm4=,size_16,color_FFFFFF,t_70" alt="asdad" style="zoom: 50%;" />
...
<center><b>辛醜年(牛) · 叁月廿柒<center><b>寫於公元貳零貳壹年伍月捌日<center><b>The end
